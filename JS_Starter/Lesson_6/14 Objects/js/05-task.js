...
function showMostFavoritedFruit() {
    ...
    users.forEach((element) => {
        if (favorites[element.favoriteFruit]) {
            favorites[element.favoriteFruit]++
        } else {
            favorites[element.favoriteFruit] = 1;
        }
    })

    console.log("Любимые фрукты:");
    let max = favorites[users[0].favoriteFruit];

    for (const favoritesKey in favorites) {
        if(favorites[favoritesKey] >= max)
        {
            max = favorites[favoritesKey];
