...
console.log("User's emails:\n\n");
for (const user of users) {
    console.log(user.email);
}

let minFriends = users[0].friends.length;
let maxFriendsArray = [];
let index = 0;

for (let i = 0 ; i < users.length; i++){
    if (users[i].friends.length >= minFriends){
        maxFriendsArray[index] =
            {
        ...
        index++;
        minFriends = users[i].friends.length;
    }
}

 console.log("\n\nИмя юзера с наибольшим количеством друзей:\n\n");

for (const maxFriend of maxFriendsArray) {
    console.log(maxFriend.fname + " " + maxFriend.lname);
}
