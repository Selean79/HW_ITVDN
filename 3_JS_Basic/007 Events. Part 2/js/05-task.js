...
Сделайте так, чтобы при наведении на имя пользователя в списке, в div#output отображались
возраст (age), баланс (balance), компания (company) и email данного пользователя
*/
...
for (let index = 0; index < users.length; index++) {
    ...
    function mouseEnterHandler() {
        let userInfo = document.querySelector("#output");
        userInfo.innerHTML =
            `User: ${+li.dataset.userNumber + 1}<br><br>
             Age: ${currentUser.age}<br><br>
             Balance: ${currentUser.balance}<br><br>
             Company: ${currentUser.company}<br><br>
